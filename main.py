...
def get_open_debtor_ids(period_id: int):
    params = 'filter=state = "open" AND $parents.$id = ' + str(period_id)
...
def run(dry_run: bool, period_id: int, batch_size: int):
    global process_start_time
    ...
    print(f'Deleting open debtors for period "{period_name}" in "{accounting_name}"', file=sys.stderr, flush=True)
    print('Press Ctrl+C to abort', file=sys.stderr, flush=True)
    sleep(10)

    # get ids of open debtors. This will take a while
    print('Waiting for open debtor ids. This will take a while.', file=sys.stderr, flush=True)
    debtor_ids = get_open_debtor_ids(period_id)
...
run(args.dry_run, args.period_id, args.batch_size)
